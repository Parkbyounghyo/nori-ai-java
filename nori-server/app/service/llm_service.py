...
class LlmService:
    ...
    async def stream_task(self, intent: str, detail: str, code: str = "",
        ...
        if intent in ("QUESTION", "SEARCH"):
            ...
            if is_modify:
                system = (
                    "당신은 ReAct(Reason+Act) 방식으로 동작하는 Java 개발 AI입니다.\n\n"
                    "절대로 '변경할 필요 없습니다', '수정 이유 없음' 같은 거부 응답을 하지 마세요.\n"
                    "반드시 구체적인 수정 코드를 작성하세요.\n\n"
                    "**Thought** (생각): 2~4문장으로 문제 분석, 제공된 코드 흐름 파악, 수정 방향.\n"
                    "**Action** (행동): 수정 코드. 파일별 구분, // ★ 추가·수정 주석.\n\n"
                    "규칙: 제공된 코드에 없는 메서드/SQL ID 만들지 마세요. 요청한 범위만 반영.\n"
                )
            else:
                system = (
                    "당신은 같은 팀 시니어 Java 개발자입니다.\n"
