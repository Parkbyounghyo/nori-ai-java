...
class LlmService:
    ...
    async def summarize_profile(self, profile_content: str) -> str:
        system = (
            ...
            "### 멀티 프로젝트 배포 구조 (server.xml 기반)\n"
            "server.xml이 포함되어 있으면 각 프로젝트가 어떤 컨텍스트 경로(예: `/`, `/gmng`)로 배포되는지 분석하세요.\n"
            "프로젝트 간의 물리적/논리적 의존성(공통 라이브러리, 공유 DB, 세션 공유 등)을 파악하세요.\n"
            "server.xml이 없으면 이 섹션은 생략.\n\n"
            "### 인프라 구조 (혈관 로직)\n"
            "인증/인가(로그인 흐름, 세션/쿠키), 예외 처리(공통 에러 페이지, 에러 로그), "
            "트랜잭션/AOP(롤백 기준, 공통 실행 로직), 로깅(파일 경로, SQL 노출 여부), "
            "외부 연동(SMTP, PG사, SSO 등) 설정이 프로필에 포함되어 있으면 요약하세요.\n"
            "각 인프라 항목의 핵심 설정값과 영향 범위를 구체적으로 쓰세요.\n"
            "인프라 섹션이 없으면 이 섹션은 생략.\n\n"
            "### 비즈니스 기능 목록\n"
            "#### 1. 기능명\n"
            ...
            "6. 중요도 순으로 정렬하세요.\n"
            "7. 한국어로만 작성.\n"
            "8. 단일 프로젝트 분석이 아니라, server.xml이 제공되면 각 프로젝트가 어떤 컨텍스트 경로로 배포되는지 포함하세요.\n"
            "9. 프로젝트 간 의존성(공통 모듈, 공유 테이블 등)이 보이면 반드시 언급하세요.\n"
        )
        messages = [
            {"role": "system", "content": system},
